...
static asciinode * build_ascii_tree_recursive(treeType * t) {
    asciinode * node;
    ...
    sprintf(node->label, "%d", t->treeVal);
    node->lablen = strlen(node->label);
    return node;
}
//Copy the tree into the ascii node structre
static asciinode * build_ascii_tree(treeType * t) {
    asciinode *node;
    if (t == NULL) return NULL;
...
static void print_ascii_tree(treeType* t) {
    asciinode *proot;
    int xmin, i;
    if (t == NULL) return;
